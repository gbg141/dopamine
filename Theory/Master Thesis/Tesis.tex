...
\begin{itemize}
	...
	We will also denote by $\Pi_C$ the operator $\mathscr{P}(\R)^{\Sspace \times \A} \rightarrow \mathcal{P}^{\Sspace \times \A}$ that applies this projection to each distribution in the return distribution function.
	
	\item Finally, C51 performs a single step of gradient descent on the Kullback-Leibler divergence (see Appendix \ref{metrics} for the definition of this metric) of the prediction $Z_t(s_t,a_t)$ from the target $\widehat{Z}_t (s_t,a_t)$:
...
TABULAR REPRESENTATIONS, CONVERGENCE! (Analysis of C51)
