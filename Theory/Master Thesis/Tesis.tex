...
\begin{comment}
The same re-expression can be done with $\E \left[X(s_{t}) \right]$. In fact, we can apply this argument recursively until arriving to the possible initial state-action pairs $(s_o,a_0)$:
\begin{equation} \label{recursiveExp}
	\E[(Y^DX)(s_{t+1})] = \sum_{(s_{t},a_{t})} \alpha(s_{t},a_{t}|s_{t+1}) \rho(s_{t},a_{t}) \cdots \sum_{(s_{0},a_{0})} \alpha(s_{0},a_{0};s_{1}) \rho(s_{0},a_{0}) \E \left[X(s_{0})\right]  
\end{equation}

Since initial states $s_0$ are independent of the considered policy, we can argue that their covariate shift ratio is 1, so every ratio distribution $X(s_0)$ can be reduced to a Delta Dirac centered at 1; this in turn makes
\begin{equation*}
	\E \left[X(s_{0}) \right] = 1 = c(s_0)
\end{equation*}
Substituting that in \ref{recursiveExp}, and comparing \ref{expDistCOP} to expansion \ref{expansionCSR} of the expected-based COP operator $Y$, which can analogously be expanded until initial state-action pairs, we conclude
\begin{equation*}
\begin{split}
	\E [(Y^D X)(s_{t+1}) ] &= \E_{s_t \sim \Bar{P}_\mu (\cdot | s_{t+1}), a_t \sim \Bar{\mu}(\cdot|s_t,s_{t+1})} \left[ \rho(s_{t},a_{t}) \E[X(s_t)] \right] \\
	&= \E_{s_t \sim \Bar{P}_\mu (\cdot | s_{t+1}), a_t \sim \Bar{\mu}(\cdot|s_t,s_{t+1})} \left[ \rho(s_{t},a_{t}) c(s_t) \right] = (Yc)(s_{t+1})
\end{split}
\end{equation*}

The expression of the density function \ref{densityDCS} can be rewritten and expanded recursively in the following way:
\begin{equation}
\begin{split}
...
Having already develop the previous formulation, we can directly define the Distributional DCOP operator%turn equation \ref{distCSR} into its discounted version as it is done in \cite{DCOPTD} in the value-based case:
...
\begin{equation*}
	V(s_{t+1}) := \log \left( X(s_{t+1}) \right), %= \log \left( \rho(S^{\mu}_{s_{t+1}},A^{\mu}_{s_{t},s_{t+1}}) \right) + V(S^{\mu}_{s_{t+1}}) 
\end{equation*}
so that
\begin{equation*}
\begin{split}
