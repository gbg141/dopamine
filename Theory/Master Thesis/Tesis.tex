...
\SetKwInput{KwOut}{\hspace{-1.5em} Output}
\SetKwInOut{Require}{\hspace{-1.5em} Require}
\Require{Estimates $W_{{\theta}_{t}}(s) = \sum_{k=1}^K p_{{\theta}_{t},k}(s) \delta_{x_k}$ and $W_{\bar{\theta}_{t}}(s) = \sum_{k=1}^K p_{\bar{\theta}_{t},k}(s) \delta_{x_k}$ for each $s\in\Sspace$}
\KwIn{A sample transition $(s_{i},a_{i}, s_{i+1})$}
    ...
    \CustomComment{Update target network if required}
    $\bar{\theta}_{t+1} \leftarrow {\theta}_{t+1}$ \textbf{ if } $t \ \% \ $\texttt{target\_update\_period}$\ =0$ \textbf{ else } $\bar{\theta}_{t}$\;
\KwOut{New estimates $W_{{\theta}_{t+1}}(s) = \sum_{k=1}^K p_{{\theta}_{t+1},k}(s) \delta_{x_k}$ and $W_{\bar{\theta}_{t+1}}(s) = \sum_{k=1}^K p_{\bar{\theta}_{t+1},k}(s) \delta_{x_k}$ for each $s \in \Sspace$  \linebreak 
