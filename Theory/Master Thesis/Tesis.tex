...
TODO: Introducir seccion que presentaras y en que orden, y principalmente de que trabajos. Y todo el contenido si es SOTA mejor di: They... In [XX]. E impersonal: It is defined... mejor evita I, we...
...
Now that we have introduced the standard expected-based RL setting, we go a step further and present its distributional counterpart, which has become the focus of many RL research since the publication of \cite{DRL}.
...
However, more difficulties arise in the \textit{control setting} when dealing with this distributional perspective; as stated in \cite{DRL}, while every optimal policy attain the same value $Q^*$ in the expected-valued case, there might be many optimal value distributions. Considering the set $\Pi^*$ of optimal policies, an optimal value distribution is defined as the value distribution of an optimal policy. Hence, the set of optimal value distributions is $\Z^* := \{ Z^{\pi^*} : \pi^* \in \Pi^* \}$.

Note that an optimal value distribution must match the full distribution of returns under some optimal policy, so that not all value distributions with expectation $Q^*$ are optimal. A \textit{distributional Bellman optimality operator} $\T$ is defined as well, but it does not behave as well as the policy evaluation operators: it is not a contraction in any usual metric between distributions, and its convergence to the set of optimal value distributions is weak.
...
\end{equation}
The notation can be simplified by denoting $C_\lambda = \Pi_{e_K^\top} C C^\top \Pi_{e_K^\top} + \lambda e_K e_K^\top $, so we can rewrite the distance definition as 
\begin{equation*}
...
\end{equation*}
\end{comment}
Looking at Equation \ref{generalizedCramer}, the idea is that the first term on the right-hand side penalizes the difference in cumulative probabilities of both distributions, whereas the second term accounts for the difference in mass; we refer to \cite{DRLlinear} for further details.

In addition to the Cramér distance generalization, authors in \cite{DRLlinear} also take into account the distribution $\xi$ according to which the states to be updated are sampled; %It can be whatever distribution, but in our case, given that we are following a policy $\pi$, we can consider it to be the stationary distribution $d_\pi$ (i.e. $d_\pi \in \Delta(\Sspace)$ such that $d_\pi = P_\pi d_\pi$; see a more detailed definition in Section \ref{COPTDsetting}).
...
\end{equation} 
it can be seen that the previously considered Cramér projection $\Pi_C$ satisfies this minimization expression\cite{DRLlinear}. The latter, which in our notation is expressed as $\Pi_{\xi,\lambda, \Phi}$, performs a $\xi$-weighted projection of any value distribution $Z$ onto the set of value distributions $\Phi$ by:
\begin{equation}
...
\end{lemma}
\end{comment}
The following Lemma \ref{projectionNonexpansion} shows an interesting result regarding the good behaviour of this projection $\Pi_{\xi,\lambda, \Phi}$
...
However, do not have any convergence guarantee for the iterates $\{X_k\}$ to some ratio distribution $X^C$, as we would like to. That is precisely what motivated us to develop the following logarithmic approach, which provides us with the desired distributional convergence.


\subsection{Log-Distributional Covariate Shift Approach}

Note that the Distributional Covariate Shift Equation \ref{distCSR} is purely multiplicative, and so it is the associated update rule in the learning setting. This complicates the analysis, possibly being one of the reasons why we could not find any convergence guarantee for the distributional DCOP-TD algorithm.
...
At this point, one might wonder why this logarithmic approach was not considered in the original value-based (D)COP-TD development; the reason is that, in general, Jensen's inequality prevents the correspondence between the multiplicative fixed point and the exponential of the log fixed point:
\begin{equation*}
...
Note that the previous result ensures that learning the additive fixed distribution $W^C$ in log space and exponentiating it would lead to a proper ratio distribution $X^C$. However, we emphasize that, so far, we do not have any guarantee that these distributional fixed point exists, neither in log space nor in the original multiplicative one. 
...
With all the main ingredients of our implementation detailed in previous sections, next we summarize the steps that conform our distributional DCOP-TD algorithms. To begin with, we show in Algorithm \ref{alg:CatDistDCOP-TD} the Categorical Distributional $\hat{\gamma}$-Discounted COP-TD main process, where 
\begin{itemize}
...
\subsection*{Abliation: Multiplicative vs Log Approach}

Abliation Seaquest:

\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{AverageEvalSeaquest.pdf}
    \caption{AverageEvalSeaquest}
    \label{fig:AverageEvalSeaquest}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.48\textwidth]{AverageEvalSeaquestLog.pdf}
    \caption{AverageEvalSeaquestLog}
    \label{fig:AverageEvalSeaquestLog}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.48\textwidth]{AverageEvalBreakoutLog.pdf}
    \caption{AverageEvalBreakoutLog}
    \label{fig:AverageEvalBreakoutLog}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.48\textwidth]{AverageEvalPongLog.pdf}
    \caption{AverageEvalPongLog}
    \label{fig:AverageEvalPongLog}
\end{figure}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.48\textwidth]{AverageEvalAsterixLog.pdf}
    \caption{AverageEvalAsterixLog}
    \label{fig:AverageEvalAsterixLog}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1\textwidth]{MeanOffpolicynessSeaquest.pdf}
    \caption{MeanOffpolicynessSeaquest}
    \label{fig:MeanOffpolicynessSeaquest}
\end{figure}
